...
class WordBoard:
	...
	def recalculate(self):
		...
		self.boardValue = {}
		for i, j in product(range(5), range(5)):
			self.boardValue[(i, j)] = self.letterVals[ord(self.board[i][j].lower()) - ord('a')]
		...
		for i in range(n):
			for j in range(m):
				...
				if self.board[i][j] == word: 
					value = self.letterMultipliers[(i, j)] * self.wordMultipliers[(i, j)] * LETTERVALS[ord(word[0].lower()) - ord('a')]
					if value > best:
						out = ([(i, j)], value, [])
						best = value
				# Non-singleton words	
				if self.board[i][j] == word[0]:
					if backtrack(i, j, word):
						wordMultiplier = reduce(lambda acc, cur : acc * self.wordMultipliers[cur], path, 1)
						value = sum(self.letterMultipliers[x] * LETTERVALS[ord(word[ind].lower()) - ord('a')] for ind, x in enumerate(path[::-1])) * wordMultiplier
						if value > best:
